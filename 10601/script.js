...
document.getElementById("searchButton").addEventListener("click", function () {
  ...
  fetch(`${API_URL}${searchValue}`)
    ...
    .then(data => {
    const latestCries = data.cries.latest
      resultDiv.innerHTML = `
        <h2>${data.name}</h2>
        ...
        <p>속도: ${data.stats[5].base_stat}</p>
          <audio src="${latestCries}" controls></audio>
        <button id="favoriteButton" onclick="addToFavorites('${data.name}')">즐겨찾기 추가</button>
      `;
    })
    .catch(error => {
